from utils import read_video, save_video, calculate_distance, draw_player_stats, convert_meters_to_pixel_distance, convert_pixel_distance_to_meters
import constants
...
def main():
    ...
    for ball_shot_ind in range(len(ball_shot_frames)-1):
        ...
        ball_shot_time_in_seconds = (end_frame - start_frame) / 30 #30fps

        # Get distance covered by the ball
        distance_covered_by_ball_pixels = calculate_distance(ball_mini_court_positions[start_frame][1], ball_mini_court_positions[end_frame][1])
        ...
        player_dpositions = player_mini_court_positions[start_frame]
        player_shot_ball = min(player_dpositions.keys(), key=lambda player_id: calculate_distance(player_dpositions[player_id], 
                                                                                                  ball_mini_court_positions[start_frame][1]))
        
        # opponent player speed
        opponent_player_id = 1 if player_shot_ball == 2 else 2
        distance_covered_by_opponent_pixels = calculate_distance(player_mini_court_positions[start_frame][opponent_player_id],
                                                                player_mini_court_positions[end_frame][opponent_player_id])
    ...
    player_stats_data_df = pd.DataFrame(player_stat_data)
